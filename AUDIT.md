...
- file and symbol names still using `jarvis_*`
- hard-coded workspace paths
- service descriptions naming Jarvis explicitly
- historical documentation files whose names and contents are still Jarvis-specific
- runtime scripts that may still assume private deployment conventions

## Required next pass before public release

1. rename files and script entrypoints from `jarvis_*` to `molt_evo_*` where appropriate
